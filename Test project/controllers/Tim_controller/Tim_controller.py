...
def intersect_protocol():
    ...
    if abs(coord2d[0]) <= 0.2 and (abs(coord2d[1]) <= 0.6 and abs(coord2d[1] >= 0.2)):
        ...
        if intersect_endzone(coord2d, destination):
            ...
            nearest_wavepoint_distance = dist(coord2d[0],coord2d[1],moveWaypoints[0][0],moveWaypoints[0][1])   
            destination = moveWaypoints[0]
            for waypoint in moveWaypoints:
                if nearest_wavepoint_distance > dist(coord2d[0],coord2d[1],waypoint[0],waypoint[1]):
...
while robot.step(TIME_STEP) != -1:
    ...
    if robot_status == "navigation_move":
        print("Navigating")
        if intersect_protocol():
            move_to_coordinate(destination)
            robot_status = "scan"
        else: 
            pass
